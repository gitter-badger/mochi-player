...
class Config(Data):
    ...
    audioFiletypes = ['*.mp3', '*.ogg', '*.wav', '*.wma', '*.m4a',
                      '*.aac', '*.ac3', '*.ape', '*.flac', '*.ra', '*.mka']
    videoFiletypes = ['*.avi', '*.divx', '*.mpg', '*.mpeg', '*.m1v', '*.m2v', '*.mpv', '*.dv', '*.3gp', '*.mov', '*.mp4', '*.m4v', '*.mqv', '*.dat', '*.vcd', '*.ogm', '*.ogv',
                      '*.asf', '*.wmv', '*.vob', '*.mkv', '*.ram', '*.flv', '*.rm', '*.ts', '*.rmvb', '*.dvr-ms', '*.m2t', '*.m2ts', '*.rec', '*.f4v', '*.hdmov', '*.webm', '*.vp8', '*.letv', '*.hlv']
    ...
    def __init__(self, argv):
        ...
        self.__dict__.update({k[2:]: v for k, v in map(
            lambda s: s.split('='), filter(lambda s: s.startswith('--'), argv))})
